...
wind_raster <- raster("wind_raster.tif") #resampled the origical 13.5GB tif file in QGIS. File is now 649KB and pixel size is 0.5.
...
# plot(selectrast_s)
 ...
 selectrast_w[samp_w[1:stop]]<-3 #set all cells in your sample (up to where you achieve energy target to 1)
 ...
 selectrast_b[samp_b[1:stop]]<-4 #set all cells in your sample (up to where you achieve energy target to 1)
...
# layer2_h_s <- mask(probrast_w, layer1_s_h, maskvalue = 0) 
 ...
 combined <- selectrast_h+selectrast_w+selectrast_s+selectrast_b
 plot(combined)
