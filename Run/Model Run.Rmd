...
for(i in 1:5){
 ...
 samp_s<-sample(nrow(probrast_s)*ncol(probrast_s), size=5, prob=xs)
...
## see how it all looks!
 ...
 layer1_s_h <- mask(probrast_h, selectrast_s, maskvalue = 0) 
 ...
 ### sample from a list of cell IDs from 1:total cells in raster -- size is number in sample, and prob is the probability of selecting any cell (values from probrast)
 samp_h<-sample(nrow(probrast_h)*ncol(probrast_h), size=5, prob=xh)
 ...
 selectrast_h[samp_h[1:stop]]<-1 #set all cells in your sample (up to where you achieve energy target to 1)
...
## see how it all looks!
 ...
 #set the masked values to zero
 #layer2_h_w <- mask(probrast_w, layer1_s_h, maskvalue = 0) 
